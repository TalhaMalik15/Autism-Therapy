...
/**
 * Show SweetAlert notification (centered)
...
function showToast(message, type = 'info', title = '') {
    ...
    // Use SweetAlert2 for centered notifications
    if (typeof Swal !== 'undefined') {
        ...
        // Fallback to basic alert if SweetAlert not loaded
        alert(`${titles[type]}: ${message}`);
    }
}
 ...
 */
async function showConfirm(title, message, confirmText = 'Yes, proceed', type = 'warning') {
    if (typeof Swal !== 'undefined') {
...
async function deleteChild(childId) {
    const confirmed = await showConfirm(
        ...
        'This will permanently delete all therapy records, sessions, and reports for this child. This action cannot be undone!',
    ...
    showLoading('Deleting child profile...');

    const response = await apiRequest(`/doctor/delete-child/${childId}`, {
        method: 'DELETE'
    });
    ...
    if (response && response.ok) {
        ...
        const error = await response.json();
        showToast(error.detail || 'Failed to delete child profile', 'error');
        return false;
    }
    return false;
}
