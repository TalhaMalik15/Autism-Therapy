...
@router.delete("/doctor/delete-child/{child_id}")
async def delete_child(child_id: str, current_user: TokenData = Depends(get_current_doctor)):
    db = get_database()
    ...
    # Delete all therapy logs for this child
    await db.therapy_logs.delete_many({"child_id": ObjectId(child_id)})
    
    # Delete child code
    await db.child_codes.delete_many({"child_id": ObjectId(child_id)})
    ...
    # Remove child from doctor's assigned children
    await db.doctors.update_one(
        {"_id": ObjectId(current_user.user_id)},
        {"$pull": {"assigned_children": ObjectId(child_id)}}
    )
        ...
        if session.get(db_field):
            ...
            if isinstance(domain_data, dict):
                ...
                def extract_ratings(data):
                    """Recursively extract rating values from nested dicts"""
                    for value in data.values():
                        if isinstance(value, dict):
                            extract_ratings(value)
                        elif isinstance(value, str) and value in score_map:
                            scores.append(score_map[value])
                
                extract_ratings(domain_data)
                if scores:
...
async def send_report_email(request: Request, current_user: TokenData = Depends(get_current_user)):
    from .email_service import send_report_email_async
    ...
    if not recipient_email or not child_id:
    ...
    # Calculate stats
    total_sessions = len(logs)
    avg_duration = sum(log.get("duration_minutes", 0) for log in logs) / max(total_sessions, 1)
    avg_mood = sum(log.get("mood_rating", 3) for log in logs) / max(total_sessions, 1)
    avg_engagement = sum(log.get("engagement_level", 3) for log in logs) / max(total_sessions, 1)
    
    # Get user info
    user = await db.doctors.find_one({"_id": ObjectId(current_user.user_id)})
    if not user:
        user = await db.parents.find_one({"_id": ObjectId(current_user.user_id)})
    
    sender_name = user.get("name", "TherapyTrack") if user else "TherapyTrack"
    
    # Build email content
    report_title = "Weekly Progress Report" if report_type == "weekly" else "Monthly Progress Report"
    
    email_html = f"""
    <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto; background: #f8f9fa; padding: 20px;">
        <div style="background: linear-gradient(135deg, {'#667eea' if report_type == 'weekly' else '#11998e'}, {'#764ba2' if report_type == 'weekly' else '#38ef7d'}); padding: 30px; border-radius: 15px 15px 0 0; text-align: center;">
            <h1 style="color: white; margin: 0; font-size: 24px;">🌈 TherapyTrack</h1>
            <p style="color: rgba(255,255,255,0.9); margin: 10px 0 0 0;">{report_title}</p>
        </div>
        
        <div style="background: white; padding: 30px; border-radius: 0 0 15px 15px; box-shadow: 0 4px 15px rgba(0,0,0,0.1);">
            <h2 style="color: #1a1a2e; margin: 0 0 20px 0;">Progress Report for {child['name']}</h2>
            
            <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 15px; margin-bottom: 25px;">
                <div style="background: #f0f7ff; padding: 20px; border-radius: 10px; text-align: center;">
                    <h3 style="color: #667eea; margin: 0; font-size: 28px;">{total_sessions}</h3>
                    <p style="color: #6b7280; margin: 5px 0 0 0; font-size: 14px;">Total Sessions</p>
                </div>
                <div style="background: #f0fff0; padding: 20px; border-radius: 10px; text-align: center;">
                    <h3 style="color: #11998e; margin: 0; font-size: 28px;">{avg_duration:.0f} min</h3>
                    <p style="color: #6b7280; margin: 5px 0 0 0; font-size: 14px;">Avg Duration</p>
                </div>
            </div>
            
            <div style="margin-bottom: 25px;">
                <h3 style="color: #1a1a2e; margin: 0 0 15px 0; font-size: 16px;">Performance Metrics</h3>
                
                <div style="margin-bottom: 10px;">
                    <div style="display: flex; justify-content: space-between; margin-bottom: 5px;">
                        <span style="color: #374151; font-size: 14px;">Mood Rating</span>
                        <span style="color: #11998e; font-weight: 600;">{avg_mood:.1f}/5</span>
                    </div>
                    <div style="background: #e5e7eb; border-radius: 10px; height: 8px; overflow: hidden;">
                        <div style="background: #11998e; height: 100%; width: {avg_mood * 20}%;"></div>
                    </div>
                </div>
                
                <div style="margin-bottom: 10px;">
                    <div style="display: flex; justify-content: space-between; margin-bottom: 5px;">
                        <span style="color: #374151; font-size: 14px;">Engagement Level</span>
                        <span style="color: #667eea; font-weight: 600;">{avg_engagement:.1f}/5</span>
                    </div>
                    <div style="background: #e5e7eb; border-radius: 10px; height: 8px; overflow: hidden;">
                        <div style="background: #667eea; height: 100%; width: {avg_engagement * 20}%;"></div>
                    </div>
                </div>
            </div>
            ...
            <div style="background: #f8f9fa; padding: 15px; border-radius: 10px; margin-bottom: 20px;">
                <p style="color: #6b7280; margin: 0; font-size: 13px; text-align: center;">
                    This report was generated by TherapyTrack on {datetime.utcnow().strftime('%B %d, %Y at %H:%M UTC')}
                </p>
            </div>
            
            <p style="color: #6b7280; margin: 0; font-size: 12px; text-align: center;">
                Sent by {sender_name} via TherapyTrack Autism Therapy Management System
            </p>
        </div>
    </div>
    """
    ...
    try:
        await send_report_email_async(
            ...
            subject=f"{report_title} - {child['name']} | TherapyTrack",
...
# Email weekly report endpoint (for frontend)
@router.post("/reports/email-weekly/{child_id}")
async def email_weekly_report(child_id: str, request: Request, current_user: TokenData = Depends(get_current_user)):
    from .email_service import send_report_email_async
    ...
    # Get recent therapy logs for the week
    week_ago = datetime.utcnow() - timedelta(days=7)
    logs = await db.therapy_logs.find({
        "child_id": ObjectId(child_id),
        "session_date": {"$gte": week_ago}
    }).sort("session_date", -1).to_list(100)
    ...
    <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto; background: #f8f9fa; padding: 20px;">
        ...
        <div style="background: white; padding: 30px; border-radius: 0 0 15px 15px; box-shadow: 0 4px 15px rgba(0,0,0,0.1);">
            ...
            <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 15px; margin-bottom: 25px;">
                <div style="background: #f0f7ff; padding: 20px; border-radius: 10px; text-align: center;">
                    <h3 style="color: #667eea; margin: 0; font-size: 28px;">{total_sessions}</h3>
                    <p style="color: #6b7280; margin: 5px 0 0 0; font-size: 14px;">Sessions This Week</p>
                </div>
                <div style="background: #f0fff0; padding: 20px; border-radius: 10px; text-align: center;">
                    <h3 style="color: #11998e; margin: 0; font-size: 28px;">{avg_duration:.0f} min</h3>
                    <p style="color: #6b7280; margin: 5px 0 0 0; font-size: 14px;">Avg Duration</p>
    ...
    try:
        await send_report_email_async(
            recipient_email=recipient_email,
            subject=f"Weekly Progress Report - {child['name']} | TherapyTrack",
            html_content=email_html
        )
        return {"success": True, "message": "Report sent successfully"}
    except Exception as e:
...
# Email monthly report endpoint (for frontend)
@router.post("/reports/email-monthly/{child_id}")
async def email_monthly_report(child_id: str, request: Request, current_user: TokenData = Depends(get_current_user)):
    from .email_service import send_report_email_async
    ...
    # Get therapy logs for the month
    month_ago = datetime.utcnow() - timedelta(days=30)
    logs = await db.therapy_logs.find({
        "child_id": ObjectId(child_id),
        "session_date": {"$gte": month_ago}
    }).sort("session_date", -1).to_list(100)
    ...
    email_html = f"""
    <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto; background: #f8f9fa; padding: 20px;">
        <div style="background: linear-gradient(135deg, #11998e, #38ef7d); padding: 30px; border-radius: 15px 15px 0 0; text-align: center;">
            <h1 style="color: white; margin: 0; font-size: 24px;">🌈 TherapyTrack</h1>
            <p style="color: rgba(255,255,255,0.9); margin: 10px 0 0 0;">Monthly Progress Report</p>
        </div>
        ...
        <div style="background: white; padding: 30px; border-radius: 0 0 15px 15px; box-shadow: 0 4px 15px rgba(0,0,0,0.1);">
            ...
            <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 15px; margin-bottom: 25px;">
                <div style="background: #f0fff0; padding: 20px; border-radius: 10px; text-align: center;">
                    <h3 style="color: #11998e; margin: 0; font-size: 28px;">{total_sessions}</h3>
                    <p style="color: #6b7280; margin: 5px 0 0 0; font-size: 14px;">Sessions This Month</p>
                </div>
                <div style="background: #f0f7ff; padding: 20px; border-radius: 10px; text-align: center;">
                    <h3 style="color: #667eea; margin: 0; font-size: 28px;">{avg_duration:.0f} min</h3>
                    <p style="color: #6b7280; margin: 5px 0 0 0; font-size: 14px;">Avg Duration</p>
                </div>
            </div>
    ...
    try:
        await send_report_email_async(
            recipient_email=recipient_email,
            subject=f"Monthly Progress Report - {child['name']} | TherapyTrack",
            html_content=email_html
        )
        return {"success": True, "message": "Report sent successfully"}
    except Exception as e:
